    ...
    def _get_grid_info(self,grid_file,hist_file):
      ...
      if (type(grid_file)==type(None))&(type(hist_file)==type(None)):
        ...
        if info[4] == 'roms':
            ...
            self.theta_s  = np.float64(info[6])
            self.theta_b  = np.float64(info[7])
            self.Tcline   = np.float64(info[8])
            ...
            for line in range(6,nline):
                dep = dep + info[line]
            dep = np.array(dep, dtype=np.float64)
        ...
        try:
          self.Vtrans=np.float64(hist.Vstretching)
          self.theta_s=np.float64(hist.theta_s)
          self.theta_b=np.float64(hist.theta_b)
          self.Tcline=np.float64(hist.Tcline)
        except AttributeError:
          try:
            self.Vtrans=np.float64(hist.variables['Vstretching'][:])
          except:
            print('variable Vtransform not found in history file. Defaulting to Vtrans=1')
            self.Vtrans=1
          self.theta_s=np.float64(hist.variables['theta_s'][:])
          self.theta_b=np.float64(hist.variables['theta_b'][:])
          self.Tcline=np.float64(hist.variables['Tcline'][:])
